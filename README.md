# Igor Leite
**Backend Developer | Ethical Hacker | Security-Driven Engineer**

---

## 👋 Who Am I?

I'm **Igor Leite** — a Backend Developer transitioning into Offensive Security (Pentest / Red Team).

I treat cybersecurity as a battlefield shaped by multiple forces. Where they collide, weaknesses are exposed and that's where I operate. My focus is to elevate system resilience, uncover what others overlook, and ensure organizations are not blindsided.

Currently building **[SGL Enterprise](https://github.com/igorleite97/sgl_enterprise)** — an enterprise procurement ERP with DDD architecture, Event Sourcing audit trails, and RBAC governance.

---

## 🎓 Certifications & Education

🎓 **CST in Information Security** — UNIP (2025-2027)  
📜 **eJPT** (INE) — In preparation  
📜 **Fortinet Certified Fundamentals** — In progress  
📜 **Ethical Hacking & Defense** — Udemy (2021)

---

## 🛠️ Tech Stack

**Backend & Security:**
```
Python (FastAPI, pytest, asyncio)
TypeScript (React, Node.js)
REST APIs, JWT, RBAC
Docker, Clean Architecture, DDD
```

**Offensive Security:**
```
Kali Linux, Nmap, Metasploit, Burp Suite
Custom Python scripts for pentest automation
OWASP Top 10, vulnerability analysis
```

---

## 🏆 Profiles & Achievements

### 🎮 Training Platforms

→ **TryHackMe** - [Profile](#) *(adicione quando criar)*  
→ **HackTheBox** - [Profile](#) *(adicione quando criar)*  
→ **PortSwigger Academy** - [Profile](#) *(opcional)*

---

## 📊 GitHub Stats

![Igor's GitHub stats](https://github-readme-stats.vercel.app/api?username=igorleite97&show_icons=true&theme=dark&hide_border=true)

![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=igorleite97&layout=compact&theme=dark&hide_border=true)

---

## 🎯 Current Focus

- 🔐 Preparing for eJPT certification
- 🏗️ Building enterprise-grade secure systems
- 🐍 Developing pentest automation scripts
- 📚 Active on TryHackMe and HackTheBox

---

## 🔗 Connect With Me

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/igor-leite-a9b839222)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:igor_leite_123@hotmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-100000?style=for-the-badge&logo=github&logoColor=white)](https://github.com/igorleite97)

---

**"Evolve without hesitation and dominate the art."**
